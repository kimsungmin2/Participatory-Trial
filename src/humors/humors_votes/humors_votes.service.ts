  ...
  export class HumorVotesService {
    ...
    private async validationAndSaveVote(
      {
        ...
        humorVoteId,
        voteFor,
      }: { userId?: number; userCode?: string; humorVoteId: number; voteFor: boolean },
      queryRunner: QueryRunner,
    ) {
      // 1. 이미 userId가 null이 아니면 userId를 이용해 찾고, 없으면 userCodoe를 이요해서 찾는다.
      const isExistingVote = userId
        ? await queryRunner.manager.findOneBy(EachHumorVote, { userId, humorVoteId })
        : await queryRunner.manager.findOneBy(EachHumorVote, { userCode, humorVoteId });
    ...
    async addVoteUserorNanUser(
      ...
      try {
        await this.validationAndSaveVote(
          { userId, userCode, humorVoteId, voteFor },
          queryRunner,
        );
    ...
    async getUserVoteCounts(humorVoteId: number) {
      const result = await this.dataSource
        ...
        .where('eachVote.humorVoteId = :humorVoteId', { humorVoteId })
