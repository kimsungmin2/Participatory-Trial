...
export class PolticalDabateHallOfFameService {
  ...
  async aggVotesViewForHallOfFame(
    ...
    const candidates = await this.polticalDebateBoardsRepository
      ...
      .select([
        ...
        'polticalDebateBoard.title',
        'polticalDebateBoard.content',
      ])
      .addSelect('polticalDebateBoard.view', 'views')
      .where('polticalDebateBoard.createdAt BETWEEN :start AND :end', {
  ...
  async aggVotesForHallOfFame(polticalDebateVotes: PolticalDebateVotes[]) {
    const { start, end } = this.getThisMonthRange();

    const candidates = await this.polticalDebateVotesRepository
      .createQueryBuilder('polticalDebateBoardVote')
      .leftJoinAndSelect(
        'polticalDebateBoardVote.polticalDebateBoards',
      ...
      .where('polticalDebateBoardVote.createdAt BETWEEN :start AND :end', {
        start: start.toISOString(),
        end: end.toISOString(),
      })
  ...
  async updateViewHallOfFameDatabase(hallOfFameData: any) {
    ...
    try {
      ...
      const newViewHallOfFameEntries = hallOfFameData.map((data) => {
        const newViewHallOfFameEntry =
          new PolticalDebateBoardsViewHallOfFames();
  ...
  async getRecentHallOfFame(paginationQueryDto: PaginationQueryDto) {
    ...
    try {
      ...
      polticalDebateHallOfFame =
        await this.polticalDebateHallOfFameRepository.find({
          skip,
          take: limit,
          order: {
            totalVotes: 'DESC',
  ...
  async getViewRecentHallOfFame(paginationQueryDto: PaginationQueryDto) {
    ...
    const totalItems =
      await this.polticalDebateBoardsViewHallOfFamesRepository.count();
    try {
      const { page, limit } = paginationQueryDto;
      const skip = (page - 1) * limit;
      polticalDebateBoardsViewHallOfFames =
        await this.polticalDebateBoardsViewHallOfFamesRepository.find({
          skip,
          take: limit,
          order: {
            totalview: 'DESC',
  ...
  async findOneByPoliteHallofFameVote(id: number) {
    const OneHallOfPoliteVote =
      await this.polticalDebateHallOfFameRepository.findOneBy({ id });
  ...
  async findOneByPoliteHallofFameView(id: number) {
    const OneHallOfPoliteView =
      await this.polticalDebateBoardsViewHallOfFamesRepository.findOneBy({
        id,
      });
