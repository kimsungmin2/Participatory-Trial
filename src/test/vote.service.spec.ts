...
describe('VotesService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        VotesService,
        {
          provide: getRepositoryToken(EachVote),
  ...
  describe('validationAndSaveVote', () => {
    it('should create a new vote if no existing vote is found', async () => {
      ...
      expect(queryRunner.manager.findOneBy).toHaveBeenCalledWith(EachVote, {
        userId: 1,
        voteId: 1,
      });

      expect(queryRunner.manager.save).toHaveBeenCalledWith(EachVote, da);
    });
    ...
    it('should update an existing vote if the preference changes', async () => {
      ...
      expect(queryRunner.manager.save).toHaveBeenCalledWith(EachVote, {
        ...existingVote,
        voteFor: false,
      });
    });
  ...
  describe('getUserVoteCounts', () => {
    it('카운트', async () => {
      ...
      const userVoteCounts = await service.getUserVoteCounts(voteId);

      expect(userVoteCounts).toEqual(expectedResult);
      expect(mockDataSource.getRepository).toHaveBeenCalledWith(EachVote);
      expect(
      ...
      ).toHaveBeenCalledWith('eachVote.voteId = :voteId', { voteId });
    });
  });
  describe('getVoteCounts', () => {
    it('카운트', async () => {
      ...
      const voteCounts = await service.getVoteCounts(voteId);
  ...
  describe('투표 업데이트', () => {
    it('업뎃', async () => {
      ...
      await service.updateVoteCounts(voteId);

      expect(mockDataSource.getRepository).toHaveBeenCalledWith(EachVote);
      expect(
        mockDataSource.getRepository().createQueryBuilder().where,
      ).toHaveBeenCalledWith('eachVote.voteId = :voteId', { voteId });
      expect(mockDataSource.getRepository).toHaveBeenCalledWith(Votes);
  ...
  it('should add vote for user or non-user', async () => {
    ...
    const result = await service.addVoteUserorNanUser(
      ip,
      userId,
      voteId,
      voteFor,
