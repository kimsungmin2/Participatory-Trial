...
export class TrialHallOfFameService {
  ...
  public async aggVotesLikeForHallOfFame(trials: Trials[]) {
  ...
  public async aggVotesViewForHallOfFame(trials: Trials[]) {
    const { start, end } = this.getThisMonthRange();
  ...
  public async aggVotesForHallOfFame(votes: Votes[]) {
    const { start, end } = this.getThisMonthRange();
  ...
  public async updateHallOfFameDatabase(hallOfFameData: any) {
    ...
    try {
      ...
      // DB에 새로운 명전 저장
      console.log('Saving', newHallOfFameEntries);
